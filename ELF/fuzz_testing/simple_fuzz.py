...
def random_nonascii_unicode_mask(alphabet, word_len=12, seq_len=30):
    ...
    """
    unicode_seq = ''
    for i in range(0, random.randint(seq_len//5, seq_len)):
        unicode_seq += ' ' + ''.join(random.choice(alphabet) for j in range(random.randint(1, word_len)))
...
def task_fill_mask(model_path:dict, samples:Iterable[Any], func_dict_all:dict):
    ...
    try:
        ...
        model_tokenizer = AutoTokenizer.from_pretrained('prajjwal1/bert-small')
        org_model_pipeline =  pipeline(model=model_path.org_model_pipeline, tokenizer = model_tokenizer, task=func_dict['task'])
        comp_model_pipeline =   pipeline(model=model_path.comp_model_pipeline, tokenizer = model_tokenizer, task=func_dict['task'])
...
def task_text_generation(model_path:dict, samples:Iterable[Any], func_dict_all:dict):
    ...
    for sample in samples:
        ...
        try:
            ...
            org_sentence_embeddings -= org_sentence_embeddings.min(1, keepdim=True)[0]
            org_sentence_embeddings /= org_sentence_embeddings.max(1, keepdim=True)[0]

            comp_sentence_embeddings -= comp_sentence_embeddings.min(1, keepdim=True)[0]
            comp_sentence_embeddings /= comp_sentence_embeddings.max(1, keepdim=True)[0]
...
include_ranges = [(int(str(i),16), int(str(j),16)) for i, j in zip(character_ranges['start'], character_ranges['end'])]
alphabet = [
        ...
        case 'fill-mask':
            ...
            for i in range(sample_numbers):
                while True:
                    ...
                    if len(sample.split('[MASK]'))>1 and len(sample.split(' '))<30:
                        break
                samples.append(sample)
...
function_array = [i for i in function_array if i]
print(len(function_array))


not_working_models = []
for i in tqdm(function_array, total=len(function_array)):
    try:
        i[0]()
    except Exception as e:
        print('*'*5, i[1]['name'], i[1]['task'])
        not_working_models.append({'name':i[1]['name'], 'task':i[1]['task'], 'reason':repr(e)})

with open('not_working_models_new', 'wb') as f:
    pickle.dump(not_working_models, f)
