...
def random_nonascii_unicode_classification(alphabet, word_len=10, seq_len=10):
    """returns a randomly generated sequence of unicde
    ...
    """
    unicode_seq = ''
    for i in range(0, random.randint(2, seq_len)):
        unicode_seq += ' ' + ''.join(random.choice(alphabet) for j in range(random.randint(1, word_len)))
    return unicode_seq
...
def task_audio_classification(model_path:dict, samples:Iterable[Any], func_dict_all:dict):
    ...
    if download_model(model_path):
        raise Exception(f'Model: {model_path } is not on s3 bucket')
        
    
    #func_dict = func_dict_all[model_path.model_name]
    func_dict = func_dict_all
    ...
    with open(model_path.model_results_dir+model_path.model_result_path+'_result.pkl', 'wb') as f:
...
def task_automatic_speech_recognition(model_path:dict, samples:Iterable[Any], func_dict_all:dict):
    ...
    try:
        ...
        org_processor = Wav2Vec2Processor.from_pretrained(model_path.org_model_pipeline)
        org_model_wav = Wav2Vec2ForCTC.from_pretrained(model_path.org_model_pipeline)

        comp_processor = Wav2Vec2Processor.from_pretrained(model_path.comp_model_pipeline)
        comp_model_wav = Wav2Vec2ForCTC.from_pretrained(model_path.comp_model_pipeline)

        for sample in samples:

            inputs = org_processor(samples[0], sampling_rate=16_000, return_tensors="pt", padding=True)
            with torch.no_grad():
                logits = org_model_wav(inputs.input_values, attention_mask=inputs.attention_mask).logits
            org_predicted_ids = torch.argmax(logits, dim=-1)

            output= org_processor.batch_decode(org_predicted_ids)[0]

            with torch.no_grad():
                logits = comp_model_wav(inputs.input_values, attention_mask=inputs.attention_mask).logits
            comp_predicted_ids = torch.argmax(logits, dim=-1)
            comp_output = comp_processor.batch_decode(comp_predicted_ids)[0]
...
def task_fill_mask(model_path:dict, samples:Iterable[Any], func_dict_all:dict):
    ...
    for sample in samples:
        ...
        try:
            ...
            sample = sample.replace('[MASK]', '<mask>')
            output = org_model_pipeline(sample)
            comp_output = comp_model_pipeline(sample)
...
def task_text_generation(model_path:dict, samples:Iterable[Any], func_dict_all:dict):
    ...
    for sample in samples:
        ...
        try:
                # Tokenize sentences
            org_encoded_input = org_model_pipeline.tokenizer(sample, truncation=True, return_tensors='pt')
            # Compute token embeddings
            with torch.no_grad():
                output = org_model_pipeline.model(**org_encoded_input)
            # Perform pooling. In this case, mean pooling.
            org_sentence_embeddings = mean_pooling(output, org_encoded_input['attention_mask'])

            comp_encoded_input = comp_model_pipeline.tokenizer(sample, truncation=True, return_tensors='pt')
            # Compute token embeddings
            with torch.no_grad():
                comp_output = comp_model_pipeline.model(**comp_encoded_input)

            # Perform pooling. In this case, mean pooling.
            comp_sentence_embeddings = mean_pooling(comp_output, comp_encoded_input['attention_mask'])
            comp_cosine_sim = util.pytorch_cos_sim(org_sentence_embeddings, comp_sentence_embeddings)
        except:
            pass
        ...
        func_dict['org_output'].append(output)
        func_dict['comp_output'].append(comp_output)
        #torch.all(output.eq(comp_output))
        org_token_embeddings = output[0] #First element of model_output contains all token embeddings
        org_input_mask_expanded = org_encoded_input['attention_mask'].unsqueeze(-1).expand(org_token_embeddings.size()).float()

        comp_token_embeddings = comp_output[0] #First element of model_output contains all token embeddings
        comp_input_mask_expanded = comp_encoded_input['attention_mask'].unsqueeze(-1).expand(comp_token_embeddings.size()).float()

        exact_emb_comp = torch.equal(org_token_embeddings,comp_token_embeddings)
        exact_mask_comp = torch.equal(org_input_mask_expanded, comp_input_mask_expanded)

        try:
            func_dict['comparison'].append({'cosine_comp':comp_cosine_sim, 
                                            'exact_emb_comp': exact_emb_comp,
                                            'exact_mask_comp':exact_mask_comp,
                                            'exact_comp':exact_emb_comp & exact_mask_comp})
        except NameError:
            func_dict['comparison'].append({'cosine_comp':0, 'exact_comp': torch.equal(output,comp_output)})
...
blacklisted_models = []
#['AlexN-xls-r-300m-pt', 'Ayoola-cdial-yoruba-test']
...
for model_idx, model_item in tqdm(df.iterrows(), total=df.shape[0]): #iterates over the dataframe
    ...
    output_dict = {}
    output_dict['idx'] = model_idx
    output_dict['sample_num'] = sample_numbers
    output_dict['inputs'] = []
    output_dict['org_output'] = []
    output_dict['comp_output'] = []
    output_dict['comparison'] = []
    output_dict['task'] = model_item['category']
    output_dict['name'] = model_item['model_name']
    output_dict['repo'] = model_item['repo_link'] #repo_link of model on huggingface
    ...
    model_paths = {'comp_model_pipeline':'models_compressed/'+model_item['repo_link'],
                    ...
                    'model_result_path': model_item['model_name'],
                     'model_name':model_item['model_name'],
                      'model_results_dir':'model_results_ray/' }
    ...
    result_path = Path(model_paths.model_results_dir+model_item['model_name']+'_result.pkl')
    ...
    match model_item['category']:
        case 'audio-classification':
            ...
            function_array[model_idx] = [partial(ray.remote(task_audio_classification).remote, model_paths, samples, output_dict), output_dict]
        case 'automatic-speech-recognition':
            ...
            function_array[model_idx] = [partial(ray.remote(task_automatic_speech_recognition).remote, model_paths, samples, output_dict), output_dict]
        case 'conversational':
            samples = []
            for i in range(sample_numbers):
                ...
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_conversational).remote, model_paths, samples, output_dict), output_dict]
        case 'feature-extraction':
            ...
            for i in range(sample_numbers):
                ...
                sample = conversational_dataset['test'][sample_id]['personas'][0]
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_feature_extraction).remote, model_paths, samples, output_dict), output_dict]
        case 'fill-mask':
            samples = []
            for i in range(sample_numbers):
                ...
                while True:
                    ...
                    if len(sample.split('[MASK]'))>1:
                        break
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_fill_mask).remote, model_paths, samples, output_dict), output_dict]
        case 'image-classification':
            samples = []
            for i in range(sample_numbers):
                sample = imagenet_list_dataset[random.randint(0,subsample_size-1)]['image']
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_image_classification).remote, model_paths, samples, output_dict), output_dict]
        case 'question-answering':
            ...
            for i in range(sample_numbers):
                ...
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_question_answering).remote, model_paths, samples, output_dict), output_dict]
        case 'summarization':
            samples = []
            for i in range(sample_numbers):
                sample = random_nonascii_unicode_summarization(alphabet)
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_summarization).remote, model_paths, samples, output_dict), output_dict]
        case 'text-classification':
            samples = []
            for i in range(sample_numbers):
                sample = random_nonascii_unicode_translation(alphabet)
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_text_classification).remote , model_paths, samples, output_dict), output_dict]
        ...
        case 'text2text-generation':
            ...
            for i in range(sample_numbers):
                ...
                sample = ' '.join(prompt_words[0:prompt_len])
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_text2text_generation).remote, model_paths, samples, output_dict),output_dict]
        case 'token-classification':
            samples = []
            for i in range(sample_numbers):
                while True:
                        sample = random_nonascii_unicode_classification(alphabet)
                        if len(sample.split(' '))<10:
                            break
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_token_classification).remote, model_paths, samples, output_dict), output_dict]
        case 'translation':
            samples = []
            for i in range(sample_numbers):
                sample = random_nonascii_unicode_translation(alphabet)
                samples.append(sample)
            function_array[model_idx] = [partial(ray.remote(task_translation).remote, model_paths, samples, output_dict), output_dict]
...
def to_iterator(obj_ids):
    while obj_ids:
        ...
        except Exception as e: #will ignore all the errors raised by the model or pipeline
           print(repr(e))
           yield None
