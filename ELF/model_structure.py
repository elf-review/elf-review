...
from ELF.hashing_deduplication import get_shared_storage_tensor_dict, model_loading_fun
import sys
sys.path.append("..")
...
'''
def save_model_structure_and_flatten_weights(model_name_list):
    layer_hash_repeat_value_set_file = model_hash_layer_folder+"hash_layer_repeat_set.pkl"
    if not os.path.exists(layer_hash_repeat_value_set_file):
        print("file", layer_hash_repeat_value_set_file, "not exists. Hash layer deduplication needed.")
        return
    ...
    print(model_path, model_name, str(model_size/MB)+" MB")
    ...
    if not os.path.exists(os.path.dirname(model_struct_weights_folder_individual)):
        ...
        if os.path.exists(model_struct_weights_folder_individual+"model_structure.pkl") and os.path.exists(model_struct_weights_folder_individual+"fl_weights/"):
            print(model_struct_weights_folder_individual, "EXISTS..")
            return
    ...
    for layer_name in model:
        ...
        if md5_hash in layer_hash_repeat_value_set:
            # to record the md5 hashed layer shape.
            model_structure[layer_name] = [1, md5_hash, weights_numpy.shape]
            layer_hash_size = os.path.getsize(model_hash_layer_folder+md5_hash+".pkl")
        ...
        if layer_dtype == torch.float16:
            ...
            save_data_file(non_fl_layer_file_item, weights_numpy)
    ...
    model_structure_size = os.path.getsize(model_structure_file)
    model_numpy_size_total += model_structure_size

    print("model_size:", model_size/MB, "model_np_file_size:", model_numpy_size_total/MB, "consis_ratio:", model_numpy_size_total/model_size)
'''
...
def save_model_structure_and_flatten_weights(model_name_list):
    ...
    for model_name in model_name_list:
        ...
        model_size = os.path.getsize(model_path)
        #print("\n", cnt, model_path, model_name, str(model_size/MB)+" MB")
        model_numpy_size_total = 0
        cnt += 1
        ...
        if not os.path.exists(os.path.dirname(model_struct_weights_folder_individual)):
            ...
            if os.path.exists(model_struct_weights_folder_individual+"model_structure.pkl") and os.path.exists(model_struct_weights_folder_individual+"fl_weights/"):
                #print(cnt, model_struct_weights_folder_individual, "EXISTS..")
                continue
